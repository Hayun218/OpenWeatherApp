...
final class WeatherHeaderView: UIView {
  ...
  func configure(with weather: CityWeather) {
    ...
    temperatureLabel.text = "\(Int(round(weather.todayForecast.currentTemp)))°"
    weatherLabel.text = weather.todayForecast.weather
    minMaxLabel.text = "최고: \(Int(round(weather.todayForecast.maxTemp)))° | 최저: \(Int(round(weather.todayForecast.minTemp)))°"
  }
}
